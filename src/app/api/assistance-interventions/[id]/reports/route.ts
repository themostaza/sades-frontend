...
// Utility per mappare le chiavi dei motivi di fallimento a label leggibili
function getFailureReasonLabel(failureReasonKey: string): string {
  const reasonMap: Record<string, string> = {
    cliente_assente: 'Cliente assente',
    materiale_mancante: 'Materiale mancante',
    materiale_dimenticato: 'Materiale dimenticato',
    mancanza_di_tempo: 'Mancanza di tempo',
    subentrata_urgenza: 'Subentrata urgenza',
    ricambio_mancante: 'Ricambio mancante',
    ricambio_avariato: 'Ricambio avariato',
    informazioni_mancanti: 'Informazioni mancanti da casa madre',
    intervento_senza_soluzioni: 'Intervento senza soluzioni',
  };

  return reasonMap[failureReasonKey] || failureReasonKey;
}
...
async function duplicateFailedInterventionBackend(
  ...
  success: boolean;
  data?: AssistanceInterventionDetail;
  error?: string;
}> {
  try {
    console.log(
      '🔄 Duplicating failed intervention (backend):',
      interventionId
    );
    ...
    const getResponse = await fetch(
      `${BASE_URL}api/assistance-interventions/${interventionId}`,
      {
        method: 'GET',
        headers,
      }
    ...
    if (!getResponse.ok) {
      ...
      throw new Error(
        `Errore nel recupero dei dati dell'intervento: ${errorText}`
      );
    }

    const originalIntervention = await getResponse.json();
    console.log(
      '📋 Dati intervento originale per duplicazione:',
    ...
    try {
      const reportResponse = await fetch(
        `${BASE_URL}api/assistance-interventions/${interventionId}/reports`,
        {
          method: 'GET',
          headers,
        }
      ...
      if (reportResponse.ok) {
        const reportData: InterventionReportSummary =
          await reportResponse.json();
        console.log('📄 Rapportino failed recuperato:', reportData.id);

        // Estrai e mappa il motivo del fallimento
      ...
      console.warn('⚠️ Impossibile recuperare rapportino failed:', reportError);
    }

    // 3. Estrai gli articoli con i loro magazzini di origine dai movements
    const articlesWithWarehouses =
      originalIntervention.connected_articles
        ?.map((art: ConnectedArticle) => {
          const fromWarehouses =
            art.movements
              ?.filter(
                (m) => String(m.to_warehouse_id) === 'CL' && m.from_warehouse_id
              )
              .map((m) => ({
        ...
        .filter(
          (art: {
            article_id: string;
            quantity: number;
            from_warehouses: Array<{ warehouse_id: string; quantity: number }>;
          }) => art.from_warehouses.length > 0
        ) || [];
    ...
    // 5. Crea il nuovo intervento VUOTO (senza articoli)
    const newInterventionPayload = {
      ...
      equipments:
        originalIntervention.connected_equipment?.map(
          (eq: Equipment) => eq.id
        ) || [],
    ...
    console.log(
      '📤 Payload nuovo intervento duplicato (senza articoli):',
    ...
    if (!postResponse.ok) {
      ...
      throw new Error(
        `Errore nella creazione del nuovo intervento: ${errorText}`
      );
    }
    ...
    // 6. AWAIT - Rimuovi articoli dall'intervento originale (tornano a magazzino)
    if (articlesWithWarehouses.length > 0) {
      console.log(
        "🔄 Step 1/2: Rimozione articoli dall'intervento originale..."
      );
      ...
      const removeArticlesPayload = {
        ...
        flg_discount_home_service:
          originalIntervention.flg_discount_home_service,
        ...
        quotation_price:
          parseFloat(String(originalIntervention.quotation_price)) || 0,
      ...
      if (!removeResponse.ok) {
        ...
        throw new Error(
          `Errore nella rimozione articoli dall'intervento originale: ${errorText}`
        );
      }

      console.log(
        "✅ Articoli rimossi dall'intervento originale (tornati a magazzino)"
      );

      // 7. AWAIT - Aggiungi articoli al nuovo intervento (prelievo da magazzino)
      ...
      const getNewResponse = await fetch(
        `${BASE_URL}api/assistance-interventions/${newIntervention.id}`,
        {
          method: 'GET',
          headers,
        }
      ...
      if (!getNewResponse.ok) {
        ...
        throw new Error(
          `Errore nel recupero del nuovo intervento: ${errorText}`
        );
      }
      ...
      const addArticlesPayload = {
        ...
        flg_discount_home_service:
          newInterventionData.flg_discount_home_service,
        ...
        quotation_price:
          parseFloat(String(newInterventionData.quotation_price)) || 0,
        ...
        equipments:
          newInterventionData.connected_equipment?.map(
            (eq: Equipment) => eq.id
          ) || [],
      ...
      if (!addResponse.ok) {
        ...
        throw new Error(
          `Errore nell'assegnazione articoli al nuovo intervento: ${errorText}`
        );
      }

      console.log(
        '✅ Articoli assegnati al nuovo intervento (prelevati da magazzino)'
      );
    } else {
      console.log('ℹ️ Nessun articolo da riassegnare');
    }

    // 8. Ricarica il nuovo intervento con tutti gli articoli aggiornati
    const getFinalResponse = await fetch(
      `${BASE_URL}api/assistance-interventions/${newIntervention.id}`,
      {
        method: 'GET',
        headers,
    ...
    if (!getFinalResponse.ok) {
      ...
      throw new Error(
        `Errore nel recupero finale del nuovo intervento: ${errorText}`
      );
    }

    const finalNewIntervention = await getFinalResponse.json();
    console.log(
      '✅ Duplicazione completata con successo:',
      finalNewIntervention.id
    );
...
export async function POST(
  ...
  try {
    ...
    console.log(
      '🔄 Proxying intervention report creation to:',
      `${BASE_URL}api/assistance-interventions/${id}/reports`
    );
    ...
    if (body.is_failed) {
      ...
      const duplicateResult = await duplicateFailedInterventionBackend(
        id,
        authHeader
      );
      ...
      if (duplicateResult.success) {
        console.log(
          '✅ Intervento duplicato con successo:',
        ...
        console.error(
          "❌ Errore nella duplicazione dell'intervento:",
...
export async function GET(
  ...
  try {
    ...
    console.log(
      '🔄 Proxying intervention report retrieval to:',
      `${BASE_URL}api/assistance-interventions/${id}/reports`
    );
