...
export default function CreaRapportino({
  isOpen,
  onClose,
  interventionData,
}: CreaRapportinoProps) {
  ...
  const [existingReport, setExistingReport] =
    useState<InterventionReportSummary | null>(null);
  ...
  const [
    ...
    setShowRequestNewInterventionDialog,
  ] = useState(false);
  const [requestNewInterventionText, setRequestNewInterventionText] =
    useState('');
  const [isSubmittingNewIntervention, setIsSubmittingNewIntervention] =
    useState(false);
  ...
  const loadGasAndCompressorTypes = async () => {
    ...
    try {
      ...
      const [gasCompressorResponse, rechargeableGasResponse] =
        await Promise.all([
      ...
      if (gasCompressorResponse.ok) {
        ...
        setGasCompressorTypes(
          Array.isArray(gasCompressorData) ? gasCompressorData : []
        );
      } else {
        const errorText = await gasCompressorResponse.text().catch(() => '');
        console.error(
          '❌ Errore nel caricamento dei tipi di compressore:',
          gasCompressorResponse.status,
          errorText
        );
      }
      ...
      if (rechargeableGasResponse.ok) {
        ...
        setRechargeableGasTypes(
          Array.isArray(rechargeableGasData) ? rechargeableGasData : []
        );
      } else {
        const errorText = await rechargeableGasResponse.text().catch(() => '');
        console.error(
          '❌ Errore nel caricamento dei tipi di gas ricaricabile:',
          rechargeableGasResponse.status,
          errorText
        );
      }
    } catch (error) {
  ...
  const checkExistingReport = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/assistance-interventions/${interventionData.id}/reports`,
        { headers }
      );
  ...
  const handleSubmitNewInterventionRequest = async () => {
    ...
    try {
      ...
      console.log(
        '📤 Payload per notifica nuovo intervento:',
        notificationPayload
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || "Errore durante l'invio della notifica"
        );
      }
      ...
      showNotification(
        'success',
        'Richiesta inviata!',
        "La richiesta di nuovo intervento è stata inviata con successo all'amministrazione."
      );
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log(
        '📋 Items filtrati:',
        equipmentItems.filter((item) => isItemEmpty(item)).length
      );
          ...
          ...(item.hasGas
            ? {
                gas_compressor_types_id:
                  getGasCompressorTypeId(item.tipologiaCompressore) || null,
                is_new_installation: item.nuovaInstallazione === 'Sì',
                rechargeable_gas_types_id:
                  getRechargeableGasTypeId(item.tipologiaGasCaricato) || null,
                ...
                ...(shouldShowRecuperoGasFields(item)
                  ? {
                      recovered_rech_gas_types_id:
                        getRechargeableGasTypeId(item.tipologiaGasRecuperato) ||
                        null,
                      qty_gas_recovered:
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || 'Errore durante la creazione del rapportino'
        );
      }
      ...
      if (reportData.duplicated_intervention) {
        console.log(
          '✅ Nuovo intervento creato automaticamente:',
          reportData.duplicated_intervention.id
        );
        showNotification(
          'success',
          'Intervento duplicato!',
        ...
        console.warn(
          '⚠️ Warning nella duplicazione:',
          reportData.duplication_warning
        );
        showNotification(
          'error',
          'Attenzione',
      ...
      alert(
        `Errore durante la creazione del rapportino: ${error instanceof Error ? error.message : 'Errore sconosciuto'}`
      );
    }
  };
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-sm font-medium text-yellow-800">
                  Rapportino già esistente
                </h3>
                <p className="mt-1 text-sm text-yellow-700">
                  Esiste già un rapportino per questo intervento (ID:{' '}
                  {existingReport.id}, Stato: {existingReport.status}).
                  Procedendo con la creazione di un nuovo rapportino, quello
                  esistente potrebbe essere sovrascritto.
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-yellow-700 mt-1">
                    È necessario specificare il motivo per cui l&apos;intervento
                    non è riuscito prima di poter creare il rapportino.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-yellow-700 mt-1">
                    Per un intervento riuscito è necessario specificare almeno
                    un&apos;apparecchiatura, un articolo utilizzato, delle note
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={oreLavoro}
                  onChange={(e) =>
                    setOreLavoro(normalizeDecimalInput(e.target.value))
                  }
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={oreViaggio}
                  onChange={(e) =>
                    setOreViaggio(normalizeDecimalInput(e.target.value))
                  }
                  onFocus={(e) => e.target.select()}
                  onClick={(e) => e.currentTarget.select()}
                  inputMode="decimal"
            ...
            >
                ...
                <div className="relative">
                  <input
                    ...
                    onClick={() => {
                      ...
                      searchEquipments(
                        item.id,
                        equipmentSearchQueries[item.id] || ''
                      );
                    }}
                    ...
                    <div className="bg-white rounded-lg p-4 md:p-6 max-w-3xl md:max-w-4xl w-full mx-4 h-[95vh] overflow-y-auto">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="border rounded-lg max-h-56 overflow-y-auto">
                          {(interventionData.connected_equipment || [])
                            .length === 0 ? (
                            <div className="px-4 py-3 text-sm text-gray-500">
                              Nessuna apparecchiatura associata
                            </div>
                          ) : (
                            (interventionData.connected_equipment || []).map(
                              (eq) => {
                                const alreadySelectedIds = equipmentItems
                                  .filter((ei) => ei.id !== item.id)
                                  .map((ei) => ei.selectedEquipment?.id)
                                  .filter(Boolean) as number[];
                                const isDisabled = alreadySelectedIds.includes(
                                  eq.id
                                );
                                return (
                                  <div
                                    ...
                                    onClick={() => {
                                      ...
                                      setShowEquipmentSelectorDialogs(
                                        (prev) => ({
                                          ...prev,
                                          [item.id]: false,
                                        })
                                  ...
                                  >
                                    ...
                                    <div className="text-xs text-gray-500">
                                      ...
                                      {eq.customer_name && (
                                        <span className="mr-2">
                                          • Cliente: {eq.customer_name}
                                        </span>
                                      )}
                                      {eq.linked_serials && (
                                        <span>
                                          • Linked: {eq.linked_serials}
                                        </span>
                                      )}
                                    </div>
                                  </div>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <label className="block text-sm font-medium text-gray-700">
                            Cerca altre apparecchiature del cliente
                          </label>
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${interventionData.customer_location_id ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-800'}`}
                          >
                            {interventionData.customer_location_id
                              ? 'Da: Destinazione'
                              : 'Da: Cliente'}
                          </span>
                        </div>
                ...
                {item.selectedArticles.length > 0 && (
                  <div className="mt-3 space-y-2">
                    ...
                    {item.selectedArticles.map((selectedArticle) => {
                      const planned = getPlannedArticleQty(
                        selectedArticle.article.id
                      );
                      const currentTotal = equipmentItems.reduce(
                        (sum, it) =>
                          sum +
              ...
              <div className="mb-6">
                ...
                <textarea
                  value={item.notes}
                  onChange={(e) =>
                    updateEquipmentItem(item.id, 'notes', e.target.value)
                  }
                  rows={3}
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      checked={item.hasGas}
                      onChange={(e) =>
                        updateEquipmentItem(item.id, 'hasGas', e.target.checked)
                      }
                      ...
                      <div className="space-y-2">
                        {[
                          ...
                          <label key={servizio} className="flex items-center">
                            <input
                              type="checkbox"
                              checked={item.serviziAggiuntivi.includes(
                                servizio
                              )}
                              onChange={() =>
                          ...
                          >
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <button
                                  onClick={() =>
                                    removeImageFromItem(item.id, image.id)
                                  }
                                  className="text-red-500 hover:text-red-700 hover:bg-red-50 p-1 rounded transition-colors"
                    ...
                    <div>
                      <S3ImageUploader
                        ...
                        onUploadFailed={(error: Error) => {
                          console.error(
                            'Error uploading equipment image:',
                            error
                          );
                          alert(
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Invia in automatico una notifica all&apos;amministrazione per
                aprire un nuovo intervento su questo cliente
              </p>
            </div>
            <button
          ...
          {interventoNonRiuscito && (
            <div>
              ...
              >
                ...
                <option value="materiale_mancante">
                  2. Materiale mancante
                </option>
                <option value="materiale_dimenticato">
                  3. Materiale dimenticato
                </option>
                <option value="mancanza_di_tempo">4. Mancanza di tempo</option>
                <option value="subentrata_urgenza">
                  5. Subentrata urgenza
                </option>
                <option value="ricambio_mancante">6. Ricambio mancante</option>
                <option value="ricambio_avariato">7. Ricambio avariato</option>
                <option value="informazioni_mancanti">
                  8. Informazioni mancanti da casa madre
                </option>
                <option value="intervento_senza_soluzioni">
                  9. Intervento senza soluzioni
                </option>
              </select>
            </div>
          )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-500">
                Se torni indietro ora, tutti i dati inseriti andranno persi. Sei
                sicuro di voler uscire senza salvare il rapportino?
              </p>
            </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-500 mb-4">
                Descrivi i dettagli della richiesta di apertura del nuovo
                intervento per il cliente {interventionData.company_name}:
              </p>
              <textarea
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={
                  isSubmittingNewIntervention ||
                  !requestNewInterventionText.trim()
                }
