...
export default function InterventionDetailsSectionDetail({
  ...
  const [allocationArticle, setAllocationArticle] =
  ...
  const getStockValue = (inv: ArticleInventory): number => {
    ...
    const stockVal =
      typeof extended.in_stock === 'number' && extended.in_stock != null
        ? extended.in_stock
  ...
  >([]);
  const [allocationEditArticleId, setAllocationEditArticleId] = useState<
    string | null
  >(null);
  ...
  const searchArticles = async (query: string = '') => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/articles?stock=1&query=${encodeURIComponent(query)}`,
        { headers }
      );
      if (response.ok) {
        const data = await response.json();
        const filteredArticles = (data.data || []).filter(
          (art: ArticleListItem) =>
            !selectedArticles.find((selected) => selected.article.id === art.id)
        );
  ...
  const buildAllocationRows = async (
    ...
    try {
      ...
      const inv: ArticleInventoryRow[] = Array.isArray(data.inventory)
        ? data.inventory
        : [];
      ...
      for (const invRow of inv) {
        const warehouseId = String(
          invRow.warehouse_id ?? invRow.warehouse ?? ''
        );
        if (warehouseId === 'CL') continue; // Escludi CL
        const stock = getStockValue(invRow as ArticleInventory);
        const description = String(
          invRow.warehouse_description ?? invRow.warehouse ?? ''
        );
        invMap.set(warehouseId, {
      ...
      for (const a of existingAllocations) {
        allocMap.set(a.warehouse_id, {
          quantity: a.quantity || 0,
          description: a.warehouse_description,
        });
      }
      ...
      const rows = Array.from(ids).map((wid) => {
        ...
        return {
          ...
          warehouse_description:
            invEntry?.description || allocEntry?.description || wid,
  ...
  const removeArticle = (articleId: string) => {
    setSelectedArticles(
      selectedArticles.filter((art) => art.article.id !== articleId)
    );
  };
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6">
      ...
      <div className="mb-6">
        ...
        <textarea
          ...
          rows={4}
          placeholder={
            isFieldsDisabled ? 'Non modificabile' : 'Inserisci note...'
          }
          ...
          {servizioDomicilio === 'Si' && (
            <div className="mt-2">
              <label className="flex items-center">
                ...
                />
                <span
                  className={`text-sm ${isFieldsDisabled ? 'text-gray-400' : 'text-gray-600'}`}
                >
                  Sconto
                </span>
              </label>
          ...
          {orarioIntervento === 'fascia_oraria' && (
            <div className="mt-3 grid grid-cols-2 gap-3">
              <input
                ...
                disabled={isFieldsDisabled}
                className={`w-full px-3 py-2 border rounded-lg ${isFieldsDisabled ? 'bg-gray-50' : ''}`}
              />
              <input
                type="time"
                value={oraFine}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <div className="flex">
            <input
              ...
              disabled={isFieldsDisabled}
              className={`flex-1 px-3 py-2 border rounded-l-lg ${isFieldsDisabled ? 'bg-gray-50' : ''} text-gray-700`}
              placeholder="Inserisci importo"
            />
            <span className="bg-gray-100 border-t border-b border-r px-3 py-2 rounded-r-lg text-gray-600">
              EUR
            </span>
          </div>
        </div>
        <div>
          ...
          <input
            ...
            placeholder={
              isFieldsDisabled ? 'Non modificabile' : 'Orario apertura'
            }
            disabled={isFieldsDisabled}
            className={`w-full px-3 py-2 border rounded-lg ${isFieldsDisabled ? 'bg-gray-50' : ''} text-gray-700`}
          />
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          {selectedCustomerId && (
            <span
              className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${destinazione ? 'bg-teal-100 text-teal-800' : 'bg-gray-100 text-gray-800'}`}
            >
          ...
          <input
            ...
            placeholder={
              !isEquipmentSearchEnabled()
                ? 'Seleziona cliente/destinazione'
          ...
          {showEquipmentDropdown && equipments.length > 0 && (
            <div className="absolute z-10 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-60 overflow-y-auto">
              {equipments.map((eq) => (
                <div
                  key={eq.id}
                  onMouseDown={() => handleEquipmentSelect(eq)}
        ...
        {isCreating && !selectedCustomerId && (
          <p className="mt-1 text-xs text-gray-500">
            Seleziona un cliente per cercare apparecchiature.
          </p>
        )}
        {isCreating &&
          selectedCustomerId &&
          ...
          !destinazione && (
            <p className="mt-1 text-xs text-gray-500">
              Seleziona una destinazione per affinare la ricerca.
            </p>
          )}
          ...
          <input
            ...
            placeholder={
              isFieldsDisabled
                ? 'Non modificabile'
                : isGammaSyncLock
                  ? 'Bloccato per sincronizzazione (-1 / +5 min di ogni ora)'
                  : 'Cerca ricambi...'
            }
          ...
          {showArticleDropdown && articles.length > 0 && (
            <div className="absolute z-10 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-60 overflow-y-auto">
              {articles.map((art) => {
                ...
                const totalStock =
                  art.inventory
                    ?.filter((inv) => String(inv.warehouse_id ?? '') !== 'CL')
                  ...
                  >
                      ...
                      {warehouseCount > 0 && (
                        <>
                          ...
                          <span className="text-blue-600 font-medium">
                            {warehouseCount} magazzin
                            {warehouseCount === 1 ? 'o' : 'i'}
                          </span>
                        </>
                      )}
                      <span>•</span>
                    ...
                    {warehouseCount > 1 && (
                      <div className="text-xs text-gray-400 mt-1">
                        Disponibile in:{' '}
                        {availableInventory
                          .map((inv) => inv.warehouse_description)
                          .join(', ')}
                      </div>
                    )}
                  </div>
        ...
        {selectedArticles.length > 0 && (
          <div className="mt-3 space-y-2">
            {selectedArticles.map((selArt) => {
              // Calcola stock totale e numero magazzini (escludendo CL)
              const availableInventory =
                selArt.article.inventory?.filter((inv) => {
                  const wid = String(inv.warehouse_id ?? '');
                  const stockVal = getStockValue(inv);
                  return wid !== 'CL' && stockVal > 0;
                }) || [];
              const totalStock =
                selArt.article.inventory
                  ?.filter((inv) => String(inv.warehouse_id ?? '') !== 'CL')
                  .reduce((total, inv) => total + getStockValue(inv), 0) || 0;
              const warehouseCount = availableInventory.length;
              ...
              return (
                ...
                >
                    ...
                    {selArt.allocations && selArt.allocations.length > 0 && (
                      <div className="mt-1 text-xs text-gray-600">
                        ...
                        {selArt.allocations.map((a, idx) => (
                          <div
                            key={`${selArt.article.id}-${a.warehouse_id}-${idx}`}
                          >
                            {a.warehouse_description}: {a.quantity}
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!isFieldsDisabled && (
                      <button
                        onClick={() => removeArticle(selArt.article.id)}
                        className="text-red-500"
                      >
                        <X size={16} />
                      </button>
                ...
                {allocationRows.length === 0 && (
                  <div className="p-6 text-center text-gray-600">
                    <div className="text-lg font-medium">
                      Nessun magazzino con disponibilità
                    </div>
                    <div className="text-sm mt-1">
                      Non ci sono magazzini con scorte disponibili per questo
                      articolo.
                    </div>
                  </div>
                )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (raw > row.max) {
                              setAllocationAlert(
                                `Quantità richiesta superiore alla disponibilità per ${row.warehouse_description}. Massimo consentito: ${row.max}.`
                              );
                            }
                            const next = Math.max(0, Math.min(row.max, raw));
                            setAllocationRows((prev) =>
                              prev.map((r, i) =>
                                i === idx ? { ...r, value: next } : r
                              )
                            );
                          }}
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => {
                    ...
                    const total = allocationRows.reduce(
                      (s, r) => s + (r.value || 0),
                      0
                    );
                    ...
                    const allocations = allocationRows
                      ...
                      .map((r) => ({
                        ...
                        warehouse_description: r.warehouse_description,
                        quantity: r.value,
                      }));
                    if (allocationEditArticleId) {
                      // Update existing entry using current selectedArticles prop
                      const next = selectedArticles.map((sa) =>
                        sa.article.id === allocationEditArticleId
                          ? {
                              ...sa,
                              quantity: total,
                              allocations,
                      ...
                      const exists = selectedArticles.find(
                        (sa) => sa.article.id === allocationArticle.id
                      );
                      if (exists) {
                        const next = selectedArticles.map((sa) =>
                          sa.article.id === allocationArticle.id
                            ? { ...sa, quantity: total, allocations }
                            : sa
                        );
                        setSelectedArticles(next);
                      } else {
