...
export default function InterventionsView({
  ...
  const [reportsData, setReportsData] = useState<
    Record<number, InterventionReportDetail>
  >({});

  // Stati per le attività di inventario
  const [activitiesData, setActivitiesData] = useState<
    Record<number, ActivityGroup>
  >({});
  ...
  const [searchTerm, setSearchTerm] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem(
        'inventario_interventions_searchTerm'
      );
      return saved || '';
    }
    return '';
  ...
  const [dateRange, setDateRange] = useState<{ from: string; to: string }>(
    () => {
      if (typeof window !== 'undefined') {
        const saved = sessionStorage.getItem(
          'inventario_interventions_dateRange'
        );
        if (saved) {
          try {
  ...
  const [selectedZone, setSelectedZone] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem(
        'inventario_interventions_selectedZone'
      );
      return saved || '';
    }
    return '';
  ...
  const [selectedTechnician, setSelectedTechnician] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem(
        'inventario_interventions_selectedTechnician'
      );
      return saved || '';
    }
    return '';
  ...
  const [selectedManualCheck, setSelectedManualCheck] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem(
        'inventario_interventions_selectedManualCheck'
      );
      return saved || '';
    }
    return '';
  ...
  const [currentPage, setCurrentPage] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem(
        'inventario_interventions_currentPage'
      );
  ...
  const fetchReportsForInterventions = async (
    ...
    // Fetch in parallelo
    const reportPromises = interventionsWithReports.map(
      async (intervention) => {
        if (!intervention.report_id) return null;

        try {
          const response = await fetch(
            `/api/intervention-reports/${intervention.report_id}`,
            { headers }
          );
          ...
          console.error(
            `Error fetching report ${intervention.report_id}:`,
            error
          );
          return null;
        }
  ...
  const fetchActivitiesForInterventions = async (
    interventions: AssistanceIntervention[]
  ) => {
    ...
    try {
      ...
      const response = await fetch(
        '/api/inventory/activities?status=to_do&limit=1000',
        { headers }
      );
  ...
  const handleCompleteActivity = async (activityId: string) => {
    ...
    try {
      ...
      const activity = selectedActivityGroup?.activities.find(
        (a) => a.id === activityId
      );
      if (!activity) {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.details || errorData.message || 'Errore nel completamento'
        );
      }
              ...
              {loading && !initialLoading ? (
                ...
                interventionsData.map((intervention, index) => (
                  <React.Fragment key={intervention.id}>
                    ...
                    <tr
                      className={`hover:bg-gray-50 transition-colors ${index > 0 ? 'border-t border-gray-200' : ''}`}
                    >
                      <td
                    ...
                    <tr className="bg-gray-25">
                      <td colSpan={8} className="px-3 sm:px-6 py-3 border-t-0">
                        {/* Note interne della chiamata */}
                        {intervention.internal_notes &&
                          intervention.internal_notes.trim() !== '' && (
                            <div className="flex items-start gap-2 mb-4 p-2 bg-amber-50 border border-amber-200 rounded-md">
                              <MessageSquare
                        ...
                        {intervention.report_id &&
                        reportsData[intervention.report_id] ? (
                          ...
                          <div className="space-y-6">
                            ...
                            <div>
                              ...
                              {(() => {
                                ...
                                const usedEquipmentIds = reportsData[
                                  intervention.report_id!
                                ].items.map((item) => item.equipment_id);
                                    ...
                                    <table className="w-full text-xs border border-gray-200 rounded-lg">
                                      <thead className="bg-gray-50">
                                        <tr>
                                          ...
                                          <th className="px-3 py-2 text-center font-medium text-gray-700 w-24">
                                            Pianificato
                                          </th>
                                          <th className="px-3 py-2 text-center font-medium text-gray-700 w-24">
                                            Utilizzato
                                          </th>
                                          <th className="px-3 py-2 text-center font-medium text-gray-700 w-32">
                                            Differenza
                                          </th>
                                        </tr>
                                      </thead>
                                      <tbody className="divide-y divide-gray-200">
                                        {Array.from(allEquipmentIds).map(
                                          (equipmentId, index) => {
                                            const planned =
                                              plannedEquipment.find(
                                                (eq) => eq.id === equipmentId
                                              );
                                            const usedItem = reportsData[
                                              intervention.report_id!
                                            ].items.find(
                                              (item) =>
                                                item.equipment_id ===
                                            ...
                                            const displayName =
                                              usedItem?.equipment_model ||
                                              planned?.model ||
                                              equipmentId ||
                                              'N/D';
                                            const displayDescription =
                                              usedItem?.equipment_description ||
                                              planned?.description;
                                            const displaySerial =
                                              usedItem?.equipment_serial_number ||
                                              planned?.serial_number;
                                            ...
                                            return (
                                              ...
                                              >
                                                ...
                                                <td className="px-3 py-3 text-center">
                                                  {diff === 0 ? (
                                                    <span
                                                      className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-green-100 text-green-800 text-xs font-medium"
                                                    ...
                                                    <span
                                                      ...
                                                      title="Utilizzato ma non pianificato"
                                                    >
                                                      <AlertTriangle
                                                        size={12}
                                                      />
                                                    ...
                                                    <span
                                                      ...
                                                      title="Pianificato ma non utilizzato"
                                                    >
                                                      <AlertCircle size={12} />
                                                      {diff}
                                                    </span>
                            ...
                            <div>
                              ...
                              {(() => {
                                ...
                                const plannedArticles =
                                  intervention.connected_articles || [];
                                const usedArticles = reportsData[
                                  intervention.report_id!
                                ].items.flatMap((item) => item.articles || []);
                                    ...
                                    <table className="w-full text-xs border border-gray-200 rounded-lg">
                                      <thead className="bg-gray-50">
                                        <tr>
                                          ...
                                          <th className="px-3 py-2 text-center font-medium text-gray-700 w-28">
                                            Attività
                                          </th>
                                        </tr>
                                      </thead>
                                      <tbody className="divide-y divide-gray-200">
                                        {Array.from(allArticleIds).map(
                                          (articleId, index) => {
                                            const planned =
                                              plannedArticles.find(
                                                (art) => art.id === articleId
                                              );
                                            const used = usedArticles.filter(
                                              (art) =>
                                            ...
                                            const plannedQty =
                                              planned?.quantity || 0;
                                            const usedQty = used.reduce(
                                              (sum, art) => sum + art.quantity,
                                              0
                                            );
                                            const diff = usedQty - plannedQty;
                                            ...
                                            return (
                                              ...
                                              >
                                                ...
                                                <td className="px-3 py-3 text-center">
                                                  <span
                                                    className={`inline-flex items-center justify-center min-w-[2rem] px-2 py-1 rounded-full ${
                                                      plannedQty > 0
                                                ...
                                                <td className="px-3 py-3 text-center">
                                                  {diff === 0 ? (
                                                    ...
                                                    <span
                                                      className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-amber-100 text-amber-800 text-xs font-medium"
                                                      title="Utilizzato più del pianificato"
                                                    >
                                                      <AlertTriangle
                                                        size={12}
                                                      />
                                                    ...
                                                    <span
                                                      className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-red-100 text-red-800 text-xs font-medium"
                                                      title="Utilizzato meno del pianificato"
                                                    >
                                                      <AlertCircle size={12} />
                                                      {diff}
                                                    </span>
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="font-medium text-gray-700">
                                  Apparecchiature (
                                  {intervention.equipment_count || 0})
                                </span>
                              </div>
                              {intervention.connected_equipment &&
                              intervention.connected_equipment.length > 0 ? (
                                <div className="space-y-2">
                                  {intervention.connected_equipment.map(
                                    (equipment, index) => (
                                      <div
                                        key={index}
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="font-medium text-gray-700">
                                  Pezzi di ricambio (
                                  {intervention.articles_count || 0})
                                </span>
                              </div>
                              {intervention.connected_articles &&
                              intervention.connected_articles.length > 0 ? (
                                <div className="space-y-2">
                                  {intervention.connected_articles.map(
                                    (article, index) => (
                                      <div
                                        key={index}
                                        className="bg-green-50 rounded p-2 border border-green-100"
                                      >
                                ...
                                <div className="text-gray-500 italic">
                                  Nessun pezzo di ricambio utilizzato
                                </div>
                              )}
                            </div>
                          </div>
