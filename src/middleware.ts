...
async function validateTokenWithRetry(
  ...
  token: string,
  maxRetries: number = 2
): Promise<{ success: boolean; userData?: UserData; shouldLogout?: boolean }> {
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `🔄 Token validation attempt ${attempt + 1}/${maxRetries + 1}`
      );
      ...
      if (response.status >= 500) {
        console.log(
          `⚠️ Server error (${response.status}) - attempt ${attempt + 1}/${maxRetries + 1}`
        );
        ...
        console.log(
          '⚠️ Server error after retries - failing safe (allowing access)'
        );
      ...
      console.error(
        `💥 Network error during token validation (attempt ${attempt + 1}):`,
        error
      );
      ...
      console.log(
        '⚠️ Network error after retries - failing safe (allowing access)'
      );
      return { success: false, shouldLogout: false };
    }
  }
